package com.example.bback

import android.app.Dialog
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.util.Log
import android.view.ViewGroup
import android.widget.RadioGroup
import android.widget.Toast
import androidx.appcompat.app.AlertDialog
import com.bumptech.glide.Glide
import com.example.bback.databinding.ActivityMainBinding
import com.example.bback.databinding.FormProductDetailBinding
import java.text.DecimalFormat

class MainActivity : AppCompatActivity() {
	private lateinit var binding: ActivityMainBinding
	private lateinit var adapterMenu: ProductMenuAdapter
	private lateinit var adapterChoice: ProductChoiceAdapter
	private lateinit var binding2: FormProductDetailBinding

	override fun onCreate(savedInstanceState: Bundle?) {
		super.onCreate(savedInstanceState)
		binding = ActivityMainBinding.inflate(layoutInflater)
		binding2 = FormProductDetailBinding.inflate(layoutInflater)
		setContentView(binding.root)
		initViews()
		Glide.with(this)
			.load(
				"data:image/jpeg;base64,"
			)
			.into(binding.backdabang)
	}

	private fun initViews() {
		binding.apply {
			adapterMenu = ProductMenuAdapter {
				menuProductClicked(it)
			}
			rvMenu.adapter = adapterMenu
			adapterMenu.submitList(initProducts)

			adapterChoice = ProductChoiceAdapter(
				choicePlusListener = {
					choicePlusClicked(it)
				}, choiceMinusListener = {
					choiceMinusClicked(it)
				}, choiceDeleteListener = {
					choiceDeleteClicked(it)
				}
			)
			rvChoice.adapter = adapterChoice
			adapterChoice.submitList(choiceProducts)
		}
	}

	private fun updateTotalPrice() {
		var totalPrice = 0
		for (p in choiceProducts) {
			totalPrice += p.price * p.count
		}
		val fmPrice = DecimalFormat("#,###")
		binding.tvTotal.setText(fmPrice.format(totalPrice))
	}

	private fun menuProductClicked(product: Product) {
		var isExist = false
		for (p in choiceProducts) {
			if (p.idx == product.idx) isExist = true
		}
		choiceProducts.forEachIndexed { i, p ->

		}
		if (isExist) {
			Toast.makeText(
				this, "이미 상품을 추가했습니다.",
				Toast.LENGTH_LONG
			).show()
		} else {
			val dialog = Dialog(this)
			binding2.drink.setText(product.title)
			binding2.price.setText(product.price.toString())

			Glide.with(this)
				.load(product.image)
				.into(binding2.dialogCommentImageview)

			if (binding2.root.getParent() != null)
				(binding2.root.getParent() as ViewGroup).removeView(binding2.root)
			dialog.setContentView(binding2.root)

			dialog.show()
			binding2.rbIce1.setOnClickListener {
				product.option = addOptions()

			}

			binding2.rbIce2.setOnClickListener {
				product.option = addOptions()
			}

			binding2.rbIce3.setOnClickListener {
				product.option = addOptions()
			}

			binding2.rbSugar1.setOnClickListener {
//            binding2.option.setText("당도 : " +binding2.Ice3.text)
				product.option = addOptions()
			}

			binding2.rbSugar2.setOnClickListener {
//            binding2.option.setText("당도 : " +binding2.suger2.text)
				product.option = addOptions()
			}

			binding2.rbSugar3.setOnClickListener {
//            binding2.option.setText("당도 : " +binding2.suger3.text)
				product.option = addOptions()
			}

			binding2.rbSugar4.setOnClickListener {
//            binding2.option.setText("당도 : " +binding2.suger4.text)
				product.option = addOptions()
			}

			binding2.rbSugar5.setOnClickListener {
				product.option = addOptions()
			}
			binding2.putmenu.setOnClickListener {
				choiceProducts.add(
					Product(
						product.idx, product.title, product.price, product.image, 1, product.option
					)
				)
				adapterChoice.notifyDataSetChanged()
				updateTotalPrice()

				dialog.dismiss()
			}

			binding2.cancelbutton.setOnClickListener {
				dialog.dismiss()

			}

		}
	}

	private fun addOptions() : String{
		var msg = ""
		var isChecked1 = false
		if (binding2.rbIce1.isChecked) {
			msg = "얼음량 : " + binding2.Ice1.text
			isChecked1 = true
		} else if (binding2.rbIce2.isChecked) {
			msg = "얼음량 : " + binding2.Ice2.text
			isChecked1 = true
		} else if (binding2.rbIce3.isChecked) {
			msg = "얼음량 : " + binding2.Ice3.text
			isChecked1 = true
		}
		if (isChecked1) {
			msg += " / "
		}

		if (binding2.rbSugar1.isChecked) {
			msg += "당도 : " + binding2.suger1.text
		} else if (binding2.rbSugar2.isChecked) {
			msg += "당도 : " + binding2.suger2.text
		} else if (binding2.rbSugar3.isChecked) {
			msg += "당도 : " + binding2.suger3.text
		} else if (binding2.rbSugar4.isChecked) {
			msg += "당도 : " + binding2.suger4.text
		} else if (binding2.rbSugar5.isChecked) {
			msg += "당도 : " + binding2.suger5.text
		}

		binding2.option.setText(msg)
		return msg
	}

	private fun choicePlusClicked(product: Product) {
		for (p in choiceProducts) {
			if (p.idx == product.idx && p.count < MAX_ORDER_COUNT) {
				p.count++
				adapterChoice.notifyDataSetChanged()
				updateTotalPrice()
			}
		}
	}

	private fun choiceMinusClicked(product: Product) {
		for (p in choiceProducts) {
			if (p.idx == product.idx && p.count > 1) {
				p.count--
				adapterChoice.notifyDataSetChanged()
				updateTotalPrice()
			}
		}
	}

	private fun choiceDeleteClicked(product: Product) {
		AlertDialog.Builder(this)
			.setTitle("[${product.title}] 선택한 상품을 삭제하시겠습니다?")
			.setPositiveButton("예") { _, _ ->
				choiceProducts.remove(product)
				adapterChoice.notifyDataSetChanged()
				updateTotalPrice()
			}
			.setNegativeButton("아니오") { dialog, id ->
				dialog.cancel()
			}
			.create().show()
	}

	companion object {
		val initProducts: List<Product> = listOf(
			Product(
				1,
				"원조빽스치노(SOFT)",
				4000,
				"https://paikdabang.com/wp-content/uploads/2018/05/%EC%9B%90%EC%A1%B0%EB%B9%BD%EC%8A%A4%EC%B9%98%EB%85%B8-SOFT-450x588.png"
			),
			Product(
				2,
				"딸기바나나빽스치노(SOFT)",
				4000,
				"https://paikdabang.com/wp-content/uploads/2018/05/%EC%99%84%EC%A0%84%EB%94%B8%EA%B8%B0%EB%B0%94%EB%82%98%EB%82%98-SOFT-450x588.png"
			),
			Product(
				3,
				"초코바나빽스치노(SOFT)",
				4000,
				"https://paikdabang.com/wp-content/uploads/2018/05/%EC%99%84%EC%A0%84%EC%B4%88%EC%BD%94%EB%B0%94%EB%82%98%EB%82%98_%EB%B9%BD%EC%8A%A4%EC%B9%98%EB%85%B8-SOFT-450x588.png"
			),
			Product(
				4,
				"피스치오빽스치(SOFT)",
				5000,
				"https://paikdabang.com/wp-content/uploads/2018/05/%ED%94%BC%EC%8A%A4%ED%83%80%EC%B9%98%EC%98%A4%EB%B9%BD%EC%8A%A4%EC%B9%98%EB%85%B8-SOFT-450x588.png"
			),
			Product(
				5,
				"퐁당치노(바닐라)",
				3500,
				"https://paikdabang.com/wp-content/uploads/2018/05/%ED%90%81%EB%8B%B9%EC%B9%98%EB%85%B8_%EB%B0%94%EB%8B%90%EB%9D%BC-450x588.png"
			),
			Product(
				6,
				"퐁당치노(원조커피)",
				3500,
				"https://paikdabang.com/wp-content/uploads/2018/05/%ED%90%81%EB%8B%B9%EC%B9%98%EB%85%B8_%EC%9B%90%EC%A1%B0-450x588.png"
			),
			Product(
				7,
				"퐁당치노(미숫가루)",
				3500,
				"https://paikdabang.com/wp-content/uploads/2018/05/%ED%90%81%EB%8B%B9%EC%B9%98%EB%85%B8_%EB%AF%B8%EC%88%AB%EA%B0%80%EB%A3%A8-450x588.png"
			),
			Product(
				8,
				"쿠키크런치빽스치노(SOFT)",
				4300,
				"https://paikdabang.com/wp-content/uploads/2018/05/%EC%BF%A0%ED%82%A4%ED%81%AC%EB%9F%B0%EC%B9%98%EB%B9%BD%EC%8A%A4%EC%B9%98%EB%85%B8-SOFT-450x588.png"
			),
		)
		val choiceProducts: ArrayList<Product> = ArrayList<Product>()
		val MAX_ORDER_COUNT = 20
	}
}